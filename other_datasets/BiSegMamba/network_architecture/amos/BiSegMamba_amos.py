...
class HybridEarlyBlock3D(nn.Module):
    def __init__(self, C, use_aniso=True):
        super().__init__()

        # Normal CNN branch: strong local + channel mixing
        ...
        # Depth-wise large-kernel branch: cheap context
        branches = [
            nn.Conv3d(C, C, 5, padding=2, groups=C, bias=False),
            nn.Conv3d(C, C, 3, padding=2, dilation=2, groups=C, bias=False),
        ]

        if use_aniso:
            branches.append(
                nn.Conv3d(C, C, kernel_size=(1, 5, 5),
                          padding=(0, 2, 2), groups=C, bias=False)
            )

        self.context = nn.ModuleList(branches)

        self.norm = nn.InstanceNorm3d(C, affine=True)
        self.act = nn.GELU()

        # Fusion gives channel interaction after concatenating both paths
        self.fuse = nn.Sequential(
        ...
        self.gamma = nn.Parameter(torch.ones(1, C, 1, 1, 1) * 1e-6)
    ...
    def forward(self, x):
        ...
        for conv in self.context:
            y_ctx = y_ctx + conv(x)
        y_ctx = self.act(self.norm(y_ctx))

        y = self.fuse(torch.cat([y_local, y_ctx], dim=1))
        y = self.se(y)
...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        for i in range(4):
            if i == 0:
                gsc = nn.Sequential(
                    *[HybridEarlyBlock3D(dims[i], use_aniso=True)
                ...
                gsc = nn.Sequential(
                    *[HybridEarlyBlock3D(dims[i], use_aniso=False)
                    for _ in range(depths[i])]
                )
                stage = None
                ...
                gsc = nn.Sequential(
                    *[HybridEarlyBlock3D(dims[i], use_aniso=False) for j in range(depths[i])]
                )
                stage = nn.Sequential(
                    *[BiMambaLayer(dim=dims[i]) for j in range(depths[i])]
                )
