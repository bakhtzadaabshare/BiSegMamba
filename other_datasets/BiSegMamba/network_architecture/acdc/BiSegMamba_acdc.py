...
class DirectionalFusion3D(nn.Module):
    """
    Fuse 3 directional feature maps with:
      1) optional softmax directional gating
      2) concatenation
      3) 1x1x1 learned mixing
      4) optional SE refinement
      5) residual-friendly small scale parameter
    """
    def __init__(self, channels, use_softmax_gate=True, use_se=True, reduction=8):
        super().__init__()
        ...
        if use_softmax_gate:
            ...
            self.gate = nn.Sequential(
                ...
                nn.GELU(),
                nn.Conv3d(hidden, 3, 1, bias=True)      # [B, 3, 1, 1, 1]
            )
        ...
        # near-identity start for stable training
    ...
    def forward(self, x1, x2, x3):
        ...
        if self.use_softmax_gate:
            ...
            weights = torch.softmax(self.gate(gate_in), dim=1)  # [B, 3, 1, 1, 1]
        ...
        y = self.mix(torch.cat([x1, x2, x3], dim=1))
...
class BiMambaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # forward + backward in one batched call
        all_seqs = torch.cat(
        ...
        def fuse_bidirectional(fwd, bwd, shape):
            y = 0.5 * (fwd + bwd.flip(1))
            return self._from_seq(y, shape)

        out1 = fuse_bidirectional(o1, o1b, (D, W, H))
        out2 = fuse_bidirectional(o2, o2b, (W, D, H))
        out3 = fuse_bidirectional(o3, o3b, (H, W, D))
...
class ProgressiveStemPatch3D(nn.Module):
    def __init__(self, in_ch, stem_ch, out_ch):
        ...
        self.conv1 = ConvGNAct(in_ch, stem_ch, k=(3,5,5), p=(1,2,2))
        self.conv2 = ConvGNAct(stem_ch, stem_ch, k=3, p=1)

        # gradual reduction
        self.down1 = nn.Sequential(
            nn.Conv3d(stem_ch, stem_ch, kernel_size=3, stride=(1,2,2), padding=1, bias=False),
            nn.InstanceNorm3d(stem_ch),
            nn.ReLU(inplace=True),
        )

        self.refine = nn.Sequential(
            nn.Conv3d(stem_ch, stem_ch, kernel_size=(1,3,3), padding=(0,1,1), groups=stem_ch, bias=False),
            nn.InstanceNorm3d(stem_ch),
            nn.GELU(),
        )
...
class FinalUpAnisoV2(nn.Module):
    def __init__(self, in_ch, out_ch, stem_ch):
        ...
        self.up1 = nn.Sequential(
            nn.ConvTranspose3d(in_ch, out_ch, kernel_size=(1,2,2), stride=(1,2,2), bias=False),
            nn.InstanceNorm3d(out_ch), 
            nn.ReLU(inplace=True),
        )
...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        self.downsample_layers.append(
            PatchMerge3D(dims[0], dims[1], merge_size=(1, 2, 2), norm="in")
        )
        self.downsample_layers.append(
...
class SegMamba(SegmentationNetwork):
    def __init__(
        ...
        self.do_ds = False   # set True later if you add deep supervision
