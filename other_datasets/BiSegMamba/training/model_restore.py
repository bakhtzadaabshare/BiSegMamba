...
import importlib
import pkgutil
from BiSegMamba.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    ...
    BiSegMamba.training.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    if len(init)>10:
        ...
        del init[2]
        del init[-2]
    search_in = join(BiSegMamba.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="BiSegMamba.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in BiSegMamba.training.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within BiSegMamba.training.network_training (search is recursive)."
