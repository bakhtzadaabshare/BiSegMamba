...
from BiSegMamba.training.data_augmentation.default_data_augmentation import default_2D_augmentation_params, \
    get_patch_size, default_3D_augmentation_params
from BiSegMamba.training.dataloading.dataset_loading import unpack_dataset
from BiSegMamba.training.network_training.Trainer_acdc import Trainer_acdc
...
from BiSegMamba.network_architecture.acdc.BiSegMamba_acdc import SegMamba  #v2 restored the original directional fusion. 
from fvcore.nn import FlopCountAnalysis

def build_bisegmamba_param_groups(model, base_lr=2e-4, mamba_lr=5e-4, weight_decay=1e-5):
    decay = []
    no_decay = []
    mamba_decay = []
    mamba_no_decay = []
    ...
    for name, param in model.named_parameters():
        ...
        if is_mamba and is_no_decay:
            mamba_no_decay.append(param)
        elif is_mamba:
            mamba_decay.append(param)
        elif is_no_decay:
    ...
    return [
        ...
        {"params": mamba_decay, "lr": mamba_lr, "weight_decay": weight_decay},
        {"params": mamba_no_decay, "lr": mamba_lr, "weight_decay": 0.0},
    ]
class BiSegMamba_trainer_acdc(Trainer_acdc):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.plans['plans_per_stage'][self.stage]['pool_op_kernel_sizes'] = [[1, 2, 2], [2, 2, 2], [2, 2, 2]]
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            ...
            output = self.network(data)
            if do_backprop and self.epoch % 20 == 0:
                self.monitor_mamba_activity(data.detach())
            del data
            l = self.loss(output, target)
    ...
    @torch.no_grad()
    def monitor_mamba_activity(self, data):
        self.network.eval()

        if isinstance(data, np.ndarray):
            data_t = maybe_to_torch(data)
        else:
            data_t = data

        if torch.cuda.is_available():
            data_t = to_cuda(data_t)
        ...
        def make_hook(name):
            def hook(module, inputs, output):
                x_in = inputs[0]
                x_out = output
                change = (x_out - x_in).abs().mean()
                base = x_in.abs().mean() + 1e-8
                stats[name] = (change / base).item()
            return hook

        handles = []
        handles.append(self.network.vit.stages[2][0].register_forward_hook(make_hook("bimamba_block0_change_ratio")))
        handles.append(self.network.vit.stages[3][0].register_forward_hook(make_hook("mamba_block0_change_ratio")))

        _ = self.network(data_t)

        for h in handles:
            h.remove()

        bi_gamma = self.network.vit.stages[2][0].fusion.gamma.detach().abs().mean().item()
        m_gamma = self.network.vit.stages[3][0].fusion.gamma.detach().abs().mean().item()

        self.print_to_log_file(
            "MambaMonitor:",
            "bi_change=", stats.get("bimamba_block0_change_ratio", None),
            "m_change=", stats.get("mamba_block0_change_ratio", None),
            "bi_gamma=", bi_gamma,
            "m_gamma=", m_gamma,
        )
    ...
    def maybe_update_lr(self, epoch=None):
        ...
        base_lr = poly_lr(ep, self.max_num_epochs, 2e-4, 0.9)
        mamba_lr = poly_lr(ep, self.max_num_epochs, 5e-4, 0.9)

        # group order from build_bisegmamba_param_groups
        self.optimizer.param_groups[0]["lr"] = base_lr
        self.optimizer.param_groups[1]["lr"] = base_lr
        self.optimizer.param_groups[2]["lr"] = mamba_lr
        self.optimizer.param_groups[3]["lr"] = mamba_lr

        self.print_to_log_file(
            "lr base:",
            np.round(base_lr, decimals=8),
            "lr mamba:",
            np.round(mamba_lr, decimals=8)
        )
