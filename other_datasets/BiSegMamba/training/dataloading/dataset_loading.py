...
from BiSegMamba.paths import preprocessing_output_dir
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                ...
                if voxels_of_that_class is not None:
                    ...
                    # Selected voxel is the center voxel.
                    # Subtract half patch size to get lower bbox coordinate.
                    bbox_x_lb = selected_voxel[0] - self.patch_size[0] // 2
                    bbox_y_lb = selected_voxel[1] - self.patch_size[1] // 2
                    bbox_z_lb = selected_voxel[2] - self.patch_size[2] // 2

                    # IMPORTANT:
                    # Clamp to BOTH lower and upper bounds.
                    # The old code only used max(lb, ...), so bbox_lb could exceed ub.
                    bbox_x_lb = int(np.clip(bbox_x_lb, lb_x, ub_x))
                    bbox_y_lb = int(np.clip(bbox_y_lb, lb_y, ub_y))
                    bbox_z_lb = int(np.clip(bbox_z_lb, lb_z, ub_z))
                else:
