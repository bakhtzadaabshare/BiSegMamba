...
from BiSegMamba.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from BiSegMamba.experiment_planning.utils import crop
from BiSegMamba.paths import *
import shutil
from BiSegMamba.utilities.task_name_id_conversion import convert_id_to_task_name
from BiSegMamba.preprocessing.sanity_checks import verify_dataset_integrity
from BiSegMamba.training.model_restore import recursive_find_python_class
...
def main():
    ...
    search_in = join(BiSegMamba.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="BiSegMamba.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "BiSegMamba.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="BiSegMamba.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "BiSegMamba.experiment_planning" % planner_name2d)
    else:
        planner_2d = None
