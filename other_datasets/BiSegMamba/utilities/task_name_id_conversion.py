...
from BiSegMamba.paths import nnFormer_raw_data, preprocessing_output_dir, nnFormer_cropped_data, network_training_output_dir
...
def convert_id_to_task_name(task_id: int):
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError("Could not find a task with the ID %d. Make sure the requested task ID exists and that "
                           ...
                           "Installation). Here are your currently defined folders:\nBiSegMamba_preprocessed=%s\nRESULTS_"
                           "FOLDER=%s\nBiSegMamba_raw_data_base=%s\nIf something is not right, adapt your environemnt "
                           "variables." %
                           (task_id,
                            os.environ.get('BiSegMamba_preprocessed') if os.environ.get('BiSegMamba_preprocessed') is not None else 'None',
                            os.environ.get('RESULTS_FOLDER') if os.environ.get('RESULTS_FOLDER') is not None else 'None',
                            os.environ.get('BiSegMamba_raw_data_base') if os.environ.get('BiSegMamba_raw_data_base') is not None else 'None',
                            ))
