...
class DirectionalFusion3D(nn.Module):   #In paper this module refers to Adaptive Directional Fusion (ADF)
    """
...
class ProgressiveStemPatch3D(nn.Module):    #In paper this module refers to PCS (Progressive Compacting Stem)
    def __init__(self, in_ch, stem_ch, out_ch):
        ...
        self.down2 = nn.Sequential(
            nn.Conv3d(stem_ch, out_ch, kernel_size=3, stride=2, padding=1, bias=False),
...
class FinalUpAnisoV2(nn.Module):     #In paper this module refers to RCH (Reconstruction Head)
    def __init__(self, in_ch, out_ch, stem_ch):
        ...
        self.up2 = nn.Sequential(
            nn.ConvTranspose3d(out_ch, out_ch, kernel_size=2, stride=2, bias=False),
            nn.InstanceNorm3d(out_ch), 
            nn.ReLU(inplace=True),
        )
...
class StrongSpatialMixer3D(nn.Module):   #This is the module referred to as MSSM (Multi-Scale Spatial Mixer) in the paper
